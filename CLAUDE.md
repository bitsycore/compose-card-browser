...
./gradlew desktopTest            # the deterministic suite (521 tests on 2026-09-13, 11 skipped)
./gradlew :androidApp:assembleDebug
./gradlew :composeApp:run        # desktop
```

The Kotlin/Native desktop port is **experimental and off by default** -- `nativeDesktop=false` in
`gradle.properties`. With it off nothing about it is configured or listed, so day-to-day work does
not need to know it exists. [`docs/NATIVE_DESKTOP.md`](docs/NATIVE_DESKTOP.md) is the whole picture.
...
- **The Kotlin/Native desktop port.** Off by default, and the Windows binary runs. Nothing past the
  first screen has been exercised on it. Do not convert anything further "to help", and do not
  remove the scaffolding because it looks unused. `docs/NATIVE_DESKTOP.md` is the whole picture.
