...
| `INFOPLIST_KEY_UISupportedInterfaceOrientations_iPad` | all four orientations                                             | An iPad app is only eligible for multitasking -- Split View, Slide Over, a freely resized window -- if it declares **all four**. With upside-down missing, iPadOS offered the two fixed shapes and nothing between them. The iPhone key is listed separately and deliberately omits it |
