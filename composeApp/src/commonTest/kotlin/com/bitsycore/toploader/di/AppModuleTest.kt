	...
	private fun graph() = startKoin {
		modules(
			...
			module {
				...
				single<com.bitsycore.toploader.data.cache.SetRecordStore> {
					com.bitsycore.toploader.data.cache.InMemorySetRecordStore()
				}
				// Metadata lives in the same database, so it is overridden on the same terms.
				single<com.bitsycore.toploader.data.cache.MetadataStore> {
					com.bitsycore.toploader.data.cache.InMemoryMetadataStore()
				}
				single<com.bitsycore.toploader.platform.LinkOpener> {
					object : com.bitsycore.toploader.platform.LinkOpener {
